...
void main() {
  ...
  testWidgets('loads native contexts through loadContexts', (tester) async {
    ...
    final expectedSdkName = Platform.isAndroid
        ? 'maven:sentry-android'
        : 'spm:sentry-cocoa';
    ...
    expect(
      appContext!.containsKey('app_name'),
      isTrue,
      reason: 'App name missing from app context',
    );
    expect(
      appContext.containsKey('app_version'),
      isTrue,
    ...
    expect(
      deviceContext!.containsKey('model'),
      isTrue,
    ...
    if (breadcrumbs!.isNotEmpty) {
      ...
      // message or data
      expect(
        crumbMap.containsKey('message') || crumbMap.containsKey('data'),
        isTrue,
    ...
    if (Platform.isAndroid) {
      ...
      if (contexts.containsKey('package')) {
        ...
        expect(
          androidPackage!['sdk_name'],
          equals(expectedSdkName),
      ...
      expect(
        deviceContext.containsKey('manufacturer'),
        isTrue,
      ...
      expect(
        appContext.containsKey('app_build'),
        isTrue,
        reason: 'App app_build missing',
      );
      final androidAppBuild = appContext['app_build'];
      expect(
        androidAppBuild is String || androidAppBuild is num,
        isTrue,
      ...
      expect(
        appContext.containsKey('app_start_time'),
        isTrue,
      ...
      expect(
        DateTime.tryParse(androidAppStart!),
        isNotNull,
        reason: 'App app_start_time is not ISO-8601',
      );
      expect(
        appContext.containsKey('permissions'),
        isTrue,
      ...
      expect(
        appContext.containsKey('app_identifier'),
        isTrue,
      ...
      expect(
        appContext.containsKey('is_split_apks'),
        isTrue,
      ...
      expect(
        deviceContext.containsKey('archs'),
        isTrue,
      ...
      expect(deviceContext['free_memory'], isA<num>());
      expect(
        deviceContext.containsKey('arch'),
        isTrue,
      ...
      // App fields
      expect(
        appContext.containsKey('build_type'),
        isTrue,
      ...
      expect(
        iosAppBuild is String || iosAppBuild is num,
        isTrue,
        reason: 'App app_build must be String or num',
      );
      expect(
      ...
      expect(
        DateTime.tryParse(appStartTime!),
        isNotNull,
        reason: 'App app_start_time is not ISO-8601',
      );
      // App version should match the platform app version
      ...
      // integration names.
      expect(
        contexts.containsKey('integrations'),
        isFalse,
  ...
  testWidgets('addBreadcrumb and clearBreadcrumbs sync to native', (
    ...
    expect(
      testCrumb['data']['map']['custom object'],
  ...
  testWidgets('loads debug images through loadDebugImages', (tester) async {
    ...
    final allDebugImages = await SentryFlutter.native?.loadDebugImages(
      SentryStackTrace(frames: const []),
    );
    ...
    final imageAddr = int.parse(
      expectedImage.imageAddr!.replaceAll('0x', ''),
      radix: 16,
    );
    ...
    final debugImageByStacktrace = await SentryFlutter.native?.loadDebugImages(
      SentryStackTrace(frames: [sentryFrame]),
    );
  ...
  testWidgets('fetchNativeAppStart returns app start data', (tester) async {
    ...
    if (Platform.isAndroid || Platform.isIOS) {
      ...
      if (appStart != null) {
        ...
        expect(
          appStart.pluginRegistrationTime,
          greaterThan(0),
    ...
    testWidgets('captureException', (tester) async {
      ...
      await restoreFlutterOnErrorAfter(() async {
        await setupSentryAndApp(
          tester,
          dsn: exampleDsn,
          beforeSendCallback: fixture.beforeSend,
        );
      ...
      final sentEvents = fixture.sentEvents.where(
        (el) => el!.eventId.toString() == event!['id'],
      );
      expect(
        sentEvents.length,
        1,
      ); // one button click should only send one error
