...
import 'package:sentry_flutter/src/screenshot/screenshot_event_processor.dart';
  ...
  group(
    ...
    () {
      ...
      testWidgets('adds screenshot attachment with canvasKit renderer', (
        ...
        await _addScreenshotAttachment(
          tester,
      ...
      testWidgets('does not add screenshot for feedback events', (
        tester,
      ) async {
        await tester.runAsync(() async {
      ...
      group('beforeCaptureScreenshot', () {
        ...
        testWidgets(
          'does not add screenshot if async beforeCapture returns false',
          (tester) async {
            fixture.options.beforeCaptureScreenshot =
                (SentryEvent event, Hint hint, bool shouldDebounce) async {
                  await Future<void>.delayed(Duration(milliseconds: 1));
        ...
        testWidgets('does add screenshot if beforeCapture throws', (
          tester,
        ) async {
      ...
      group("debounce", () {
        ...
        testWidgets("adds screenshots after debounce timeframe", (
          tester,
        ) async {
          // Run with real async https://stackoverflow.com/a/54021863
          await tester.runAsync(() async {
    ...
    skip: kIsWasm
        ? 'ScreenshotEventProcessor tests in WASM currently results in an error: '
              'NotReadableError: Failed to execute convertToBlob on OffscreenCanvas: Readback of the source. '
              'Note: this error only occurs in the test suite.'
        : false,
  );
}
