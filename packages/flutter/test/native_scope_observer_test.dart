...
void main() {
  ...
  test('removeContextsCalls', () async {
    ...
    expect(
      verify(mock.removeContexts(captureAny)).captured.single,
      'fixture-key',
    );
  });
  ...
  test(
    ...
    () async {
      ...
      verify(
        mock.setContexts(SentryDevice.type, sentryDevice.toJson()),
      ).called(1);
      ...
      verify(
        mock.setContexts(SentryBrowser.type, sentryBrowser.toJson()),
      ).called(1);

      final sentryCulture = SentryCulture(
        locale: 'en-US',
        timezone: 'America/New_York',
      );
      await sut.setContexts(SentryCulture.type, sentryCulture);
      verify(
        mock.setContexts(SentryCulture.type, sentryCulture.toJson()),
      ).called(1);
