...
@implementation SentryFlutterReplayRecorderFFI {
  SentryReplayCaptureCallback callback;
}

- (instancetype _Nonnull)initWithCallback:(SentryReplayCaptureCallback)callbackParam {
  if (self = [super init]) {
    self->callback = [callbackParam copy];
  }
  return self;
}
...
- (void)imageWithView:(UIView *_Nonnull)view
  ...
  if (self->callback == nil) {
    NSLog(@"SentryFlutterReplayRecorderFFI has no callback. "
          @"Cannot capture a replay screenshot.");
    return;
  }

  self->callback(replayId, replayIsBuffering, ^(id value) {
    if (value == nil) {
      NSLog(@"SentryFlutterReplayRecorderFFI received null result. "
            @"Cannot capture a replay screenshot.");
    } else if ([value isKindOfClass:[NSDictionary class]]) {
      NSDictionary *dict = (NSDictionary *)value;
      long address = ((NSNumber *)dict[@"address"]).longValue;
      ...
      FlutterError *error = (FlutterError *)value;
      NSLog(@"SentryFlutterReplayRecorderFFI received an error: %@. "
            @"Cannot capture a replay screenshot.",
            error.message);
      return;
    } else {
      NSLog(@"SentryFlutterReplayRecorderFFI received an unexpected result. "
            @"Cannot capture a replay screenshot.");
    }
  });
}
