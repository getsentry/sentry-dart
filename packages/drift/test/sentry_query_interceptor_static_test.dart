...
import 'package:sentry_drift/src/utils/constants.dart' as drift_constants;
  ...
  group('$SentryQueryInterceptor with static span backend', () {
    ...
    group('close operations', () {
      ...
      test('error case adds error span', () async {
        final exception = Exception('test');
        final queryExecutor = MockQueryExecutor();
        when(queryExecutor.ensureOpen(any))
    ...
    group('transaction operations', () {
      test('without transaction, spans are added to active scope span',
          () async {
        final sut = fixture.getSut();
        final db = AppDatabase(NativeDatabase.memory().interceptWith(sut));
      ...
      test('successful commit adds insert spans', () async {
        ...
        final insertSpanCount = tx.children
            .where(
              (element) =>
                  element.context.description == expectedInsertStatement,
            )
            .length;
      ...
      test('successful commit adds delete spans', () async {
        ...
        final deleteSpanCount = tx.children
            .where(
              (element) =>
                  element.context.description == expectedDeleteStatement,
            )
            .length;
      ...
      test('successful commit adds custom query spans', () async {
        ...
        final customSpanCount = tx.children
            .where(
              (element) =>
                  element.context.description == expectedSelectStatement,
            )
            .length;
