...
if [[ -n ${CI:+x} ]]; then
    ...
    curl -Lv https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_3.19.0-stable.zip --output /tmp/flutter.zip
...
dart pub add 'dev:ffigen:12.0.0'
...
sed -i.bak 's|objc.NSString descriptionWithLocale_(objc.NSObject? locale)|objc.NSString descriptionWithLocale_(objc.ObjCObjectBase? locale)|g' lib/src/native/cocoa/binding.dart
