...
class WebSessionUpdater implements BeforeSendEventObserver {
  ...
  FutureOr<void> onBeforeSendEvent(SentryEvent event, Hint hint) async {
    ...
    // Check if SentryNavigatorObserver is registered
    if (!_options.navigatorObserverRegistered) {
      _options.logger(
        SentryLevel.info,
        'SentryNavigatorObserver is required for web session tracking but not registered.',
        logger: '$WebSessionUpdater',
      );
      return;
    }
    ...
    if (session == null) {
      _options.logger(
        SentryLevel.warning,
        'No active session found. Make sure SentryNavigatorObserver is registered.',
        logger: '$WebSessionUpdater',
      );
      return;
    }
