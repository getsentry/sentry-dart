import 'package:sentry/src/utils/url_utils.dart';
...
void main() {
  ...
  test('splits url', () {
    final sanitizedUri =
        UrlUtils.parse("https://sentry.io?q=1&s=2&token=secret#top");
  ...
  test('splits relative url', () {
    final sanitizedUri = UrlUtils.parse("/users/1?q=1&s=2&token=secret#top");
  ...
  test('splits relative root url', () {
    final sanitizedUri = UrlUtils.parse("/?q=1&s=2&token=secret#top");
  ...
  test('splits relative url with query only', () {
    final sanitizedUri = UrlUtils.parse("/users/1?q=1&s=2&token=secret");
    expect(sanitizedUri?.url, "/users/1");
    expect(sanitizedUri?.query, "q=1&s=2&token=secret");
  ...
  test('splits relative url with fragment only', () {
    final sanitizedUri = UrlUtils.parse("/users/1#top");
    expect(sanitizedUri?.url, "/users/1");
  ...
  test('strips user info with user and password without query', () {
    final sanitizedUri = UrlUtils.parse("https://user:password@sentry.io#top");
    expect(sanitizedUri?.url, "https://[Filtered]:[Filtered]@sentry.io");
    expect(sanitizedUri?.query, isNull);
    expect(sanitizedUri?.fragment, "top");
  });
  ...
  test('splits without query', () {
    final sanitizedUri = UrlUtils.parse("https://sentry.io#top");
    expect(sanitizedUri?.url, "https://sentry.io");
    expect(sanitizedUri?.query, isNull);
    expect(sanitizedUri?.fragment, "top");
  });
  ...
  test('strips user info with user and password without fragment', () {
    final sanitizedUri =
        UrlUtils.parse("https://user:password@sentry.io?q=1&s=2&token=secret");
    expect(sanitizedUri?.url, "https://[Filtered]:[Filtered]@sentry.io");
    expect(sanitizedUri?.query, "q=1&s=2&token=secret");
    expect(sanitizedUri?.fragment, isNull);
  });
  ...
  test('strips user info with user and password without query or fragment', () {
    final sanitizedUri = UrlUtils.parse("https://user:password@sentry.io");
    expect(sanitizedUri?.url, "https://[Filtered]:[Filtered]@sentry.io");
    expect(sanitizedUri?.query, isNull);
    expect(sanitizedUri?.fragment, isNull);
  });
  ...
  test('splits url without query or fragment and no authority', () {
    final sanitizedUri = UrlUtils.parse("https://sentry.io");
    expect(sanitizedUri?.url, "https://sentry.io");
    expect(sanitizedUri?.query, isNull);
    expect(sanitizedUri?.fragment, isNull);
  });
  ...
  test('strips user info with user only', () {
    final sanitizedUri =
        UrlUtils.parse("https://user@sentry.io?q=1&s=2&token=secret#top");
  ...
  test('no details extracted with query after fragment without authority', () {
    final sanitizedUri =
        UrlUtils.parse("https://sentry.io#fragment?q=1&s=2&token=secret");
    expect(sanitizedUri?.url, isNull);
    expect(sanitizedUri?.query, isNull);
    expect(sanitizedUri?.fragment, isNull);
  });
  ...
  test('no details extracted from malformed url', () {
    final sanitizedUri =
        UrlUtils.parse("htps://user@sentry.io#fragment?q=1&s=2&token=secret");
    expect(sanitizedUri?.url, isNull);
    expect(sanitizedUri?.query, isNull);
    expect(sanitizedUri?.fragment, isNull);
  });
