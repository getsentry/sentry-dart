...
void main() {
  ...
  test('listFlutterVersions() returns a stable list', () async {
    ...
    expect(
      versions.map((v) => v.tagName),
      equals(['v1.16.3', 'v1.16.2', 'v1.16.1']),
    );
  });

  test(
    'listFlutterVersions() fetches items across multiple API page requests',
    () async {
  ...
  test('downloadAndExtractTo() extracts a zip file', () async {
    ...
    expect(
      fs.file('$path/sky_engine/sky_engine/README.md').readAsStringSync(),
